          ...
          <ul class="nav-main">
            ...
            <li class="nav-main-item{{ request()->is('pages/*') ? ' open' : '' }}">
              ...
              <ul class="nav-main-submenu">
                ...
                <li class="nav-main-item">
                    <a class="nav-main-link{{ request()->routeIs('main-heads.index') ? ' active' : '' }}" href="{{ route('main-heads.index') }}">
                        <span class="nav-main-link-name">Main Head</span>
                      </a>
                </li>
                <li class="nav-main-item">
                    <a class="nav-main-link{{ request()->routeIs('companies.index') ? ' active' : '' }}" href="{{ route('companies.index') }}">
                        <span class="nav-main-link-name">Company</span>
                      </a>
                </li>
                <li class="nav-main-item">
                    <a class="nav-main-link{{ request()->routeIs('groups.index') ? ' active' : '' }}" href="{{ route('groups.index') }}">
            ...
            <li class="nav-main-item open">
                <a class="nav-main-link{{ request()->is('audit-logs.index') ? ' active' : '' }}" href="{{ route(name: 'audit-logs.index') }}">
...
{{--  <script>
  const layoutOptions = {
    ...
    layout: {
      default: [
        "{tab} ض ص ث ق ف غ ع ہ خ ح ج د",
        "{capslock} ش س ی ب ل ا ت ن م ک ط",
        "{shift} ئ ء ؤ ر لا ى ة و ز ظ {bksp}",
        "{space}"
      ],
      shift: [
        "{tab} ۱ ۲ ۳ ۴ ۵ ۶ ۷ ۸ ۹ ۰ - =",
        "{capslock} ! @ # $ % ^ & * ( ) _ +",
        "{shift} َ ً ُ ٌ ِ ٍ ْ ـ ٰ ٖ ٗ ٘ ٙ ٚ {bksp}",
        "{space}"
      ]
    },
    display: {
  ...
  function createKeyboard(input, keyboardId, keyboardDiv) {
    if (keyboards[keyboardId]) return; // already initialized
  
    shiftStates[keyboardId] = false;
  
    keyboards[keyboardId] = new SimpleKeyboard.default({
      ...layoutOptions,
      rootElement: keyboardDiv,
      onChange: inputValue => {
      ...
      onKeyPress: button => {
        if (button === "{bksp}") {
          ...
          shiftStates[keyboardId] = !shiftStates[keyboardId];
          keyboards[keyboardId].setOptions({
            layoutName: shiftStates[keyboardId] ? "shift" : "default"
          });
          return;
        } else {
          input.value += button;
        ...
        // Keep keyboard state in sync with input
        keyboards[keyboardId].setInput(input.value);
      }
    });
  }

  window.addEventListener("DOMContentLoaded", () => {
    document.querySelectorAll(".input-urdu").forEach(input => {
      const keyboardId = input.getAttribute("data-keyboard-id");
      const keyboardDiv = document.getElementById(keyboardId);

      if (!keyboardDiv) {
        console.error(`Keyboard container not found for ID: ${keyboardId}`);
        return;
      }

      // Initialize keyboard
      createKeyboard(input, keyboardId, keyboardDiv);

      input.addEventListener("focus", (e) => {
        hideAllKeyboards();
        // Only show the keyboard for this specific input
        const targetKeyboardId = e.target.getAttribute("data-keyboard-id");
        const targetKeyboard = document.getElementById(targetKeyboardId);
        if (targetKeyboard) {
          targetKeyboard.style.display = "block";
          keyboards[targetKeyboardId].setInput(input.value || "");
        }
      });

      input.addEventListener("blur", () => {
        // Small delay to allow for keyboard clicks
        setTimeout(() => {
          if (!keyboardDiv.matches(":hover")) {
      ...
      keyboardDiv.addEventListener("mousedown", e => {
        e.preventDefault(); // Don't lose focus when clicking on the keyboard
      });
    });

    // Close keyboard when clicking outside
    document.addEventListener("click", (e) => {
      if (!e.target.closest(".input-urdu") && !e.target.closest(".keyboard-container")) {
        hideAllKeyboards();
      }
    });

    hideAllKeyboards(); // Hide all on load
  });
</script>  --}}
...
{{--  <script>
    document.addEventListener("DOMContentLoaded", function () {
        ...
        const myKeyboard = new Keyboard({
            onChange: input => {
                if (currentInput) {
                    ...
                    // Trigger input event for any validation or listeners
            ...
            layout: {
                default: [
                    ...
                    "{tab} ض ص ث ق ف غ ع ہ خ ح ج د",
                    "{lock} ش س ی ب ل ا ت ن م ک گ چ ط",
                    "ظ ژ ذ ڈ ر ز و پ ڑ ء ئ ے",
                    "{shift} ٭ ، ؛ : ؟ ۔ {enter}",
                    "{space}"
                ],
                shift: [
                    "~ ! @ # $ % ^ & * ( ) {backspace}",
                    "{tab} ؂ إ أ آ ؤ ﷲ ة ٹ ں ۃ ح ج",
                    "{lock} ۓ ﯼ ۅ ۆ ۄ ۞ ۩ ۝ ڤ ڑ چ ت",
                    "` + = _ [ ] { } \\ | \" '",
                    "{shift} ٪ / < > × ؛ : {enter}",
                    "{space}"
                ]
        ...
        // Handle all Urdu input fields
        document.querySelectorAll(".input-urdu").forEach(input => {
            input.addEventListener("focus", function() {
                currentInput = this;
                keyboardEl.style.display = "block";
                myKeyboard.setInput(this.value);

                // Position keyboard near the input field
                const rect = this.getBoundingClientRect();
                keyboardEl.style.position = "absolute";
                keyboardEl.style.left = `${rect.left}px`;
                keyboardEl.style.top = `${rect.bottom + 5}px`;
            });
        ...
        document.addEventListener("click", function(e) {
            if (!keyboardEl.contains(e.target)) {
                const isUrduInput = e.target.classList.contains("input-urdu") ||
                                  e.target.closest(".input-urdu");
                if (!isUrduInput) {
        ...
        // Prevent keyboard from closing when clicking on it
